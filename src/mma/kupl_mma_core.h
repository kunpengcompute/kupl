    ...
    auto process_dim = [&](auto index_constant) {
        ...
        if constexpr (is_int_constexpr<std::decay_t<decltype(coord.template get<Index>())>>::value) {
            static_assert(((std::remove_reference_t<decltype(coord.template get<Index>())>::val <
                          ...
                          "Coord must less than Shape with the same index");
            ...
            if (kupl_likely(coord.template get<Index>() <
                            std::remove_reference_t<decltype(shape.template get<Index>())>::val)) {
                result +=
...
template <typename StoreAtom, typename Shape>
TiledStore<StoreAtom, Shape> make_tiled_store([[maybe_unused]] StoreAtom store_atom, [[maybe_unused]] Shape atom_shape)
{
...
template <typename TiledCopy, typename dtypeS, typename LayoutS>
void copy(TiledCopy tiled_copy, Tensor<dtypeS, LayoutS> src)
{
    tiled_copy.call(src);
}
...
static kupl_always_inline void kupl_prefetch_L1(const void *data)
{
...
static kupl_always_inline void kupl_prefetch_L2(const void *data)
{
